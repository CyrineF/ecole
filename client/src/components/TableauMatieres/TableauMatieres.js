...
export default function TableauMatieres(props) {
  ...
  const handleAdd = () => {
    if (!libele || !chargeHoraire) {
      ...
      setMsg("")
      fetch('http://localhost:4000/matiere', {
        method: 'POST',
        headers: {
    ...
    <div className={classes.tableResponsive}>
      <Table className={classes.table}>
        ...
        <TableBody>
            ...
            return (
              <TableRow className={classes.tableBodyRow}>
                ...
                <TableCell key={key} className="cell">
                  <Delete id={prop._id} nom={prop.label} niveau={prop.niveau} />
                </TableCell>
                <TableCell key={key} className="cell">
          ...
          <TableRow>
            <TableCell colSpan={4}>
              <Tooltip title="Ajouter" aria-label="Ajouter" onClick={handleOpen}>
                <Fab color="secondary" className={classes.absolute} style={{ float: "right", backgroundColor: "#8e24aa" }}>
                  <AddIcon />
                </Fab>
              </Tooltip>
              <Dialog open={open} onClose={handleClose} aria-labelledby="form-dialog-title">
                <DialogTitle id="form-dialog-title">Nouvelle Matière</DialogTitle>
                <DialogContent>
                  <DialogContentText>
                    Donnez les informations suivantes pour ajouter une nouvelle matière pour le niveau {props.i}
                  </DialogContentText>
